...
name: PRD생성
version: "1.1.1"
description: AI 코딩 에이전트가 완벽하게 실행할 수 있는 기계 해독 가능한 PRD(Product Requirements Document)를 생성한다.
             적응형 재귀 인터뷰로 요구사항을 발굴하고, JTBD 분석, 환각 방지, YAML+Mermaid 구조화를 통해
             실행 가능한 명세서를 만든다. "PRD 만들어줘", "기획서 작성해줘", "요구사항 정리해줘"라고 하면 실행된다.
---
...
- **언제 사용하는가**: 사용자의 추상적인 비즈니스 아이디어를 AI 에이전트가 코딩할 수 있는 구체적인 명세서(PRD)로 정의할 때 사용합니다.
- **실행 트리거 (Trigger Prompts)**: "PRD 만들어줘", "기획서 작성해줘", "요구사항 정리해줘" 등의 발화.
- **최소 산출물 기준 (Minimum Output Criteria)**:
  - `PRD_v1.0.md` 파일 (Must Have 기능 명세, ERD, 유저플로우 Mermaid 다이어그램, AI Evals 검증 리스트 포함).
- **완료 조건 (Definition of Done)**:
  - 기계 검증 가능한 Given-When-Then 인수조건 명시.
  - Out-of-Scope 제외 범위 3개 이상 작성 완료.
