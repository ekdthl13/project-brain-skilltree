...
### Added
- Automated `SKILL_INDEX` version-row validation guard to prevent version drift between source files and catalog.
- Document operations diet and history archive to keep the root directory clean.
- Safety rules for `project-manager` to enforce overwrite-slot policies.

### Changed
- Refactored `코딩가이드` (v1.7.0) and `암행어사` (v2.1.0) into split files (SKILL, REFERENCES, CHECKLIST) for lower context overhead.
- Regenerated all compiled adapters for Antigravity, Codex, and Claude Code.
